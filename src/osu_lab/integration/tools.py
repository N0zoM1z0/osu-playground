...
from osu_lab.beatmap.io import load_ir_json, package_osz, parse_osu, write_osu
from osu_lab.beatmap.validate import verify_beatmap
from osu_lab.core.models import BeatmapIR, StyleTarget
...
def build_style_profile_tool(ref_maps: list[str]) -> dict[str, object]:
    return build_style_profile(ref_maps).__dict__
...
def plan_live_play_tool(map_path_or_tosu_context: str, profile: str) -> dict[str, object]:
    return plan_live_play(map_path_or_tosu_context, profile=profile).__dict__


def ai_draft_tool(backend: str, audio_path: str) -> dict[str, object]:
    return draft_with_backend(backend, audio_path)
